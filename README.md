...
- **Negative search:** a term that starts with `!` excludes everything that contains it instead of searching for it — in a column filter and in the search box of the list alike. `!7.16` shows every router that is not on 7.16, and a bare `!` clears the search again. The term is taken literally, so a `.` or a `[` in it stays a character. The column filters also match without regard to case now, the way the search box always did.
